...
class TestSessionsClient:
    ...
    async def test_create_session(self, async_client, mock_http, mock_resolve):
        ...
        mock_http.post("/v1/projects/p1/sessions").respond(
            json={
                "id": "s1",
                "project_id": "p1",
                "created_by": "u1",
    ...
    async def test_get_session(self, async_client, mock_http, mock_resolve):
        ...
        session_id = "s1"
        mock_http.get(f"/v1/projects/p1/sessions/{session_id}").respond(
            json={
                "id": session_id,
                "project_id": "p1",
                "created_by": "u1",
    ...
    async def test_list_sessions(self, async_client, mock_http, mock_resolve):
        ...
        mock_http.get("/v1/projects/p1/sessions").respond(
            json={
                "data": [
                    {
                        "id": "s1",
    ...
    @pytest.mark.asyncio
    async def test_list_sessions_with_cursor(
    ...
    async def test_get_messages(self, async_client, mock_http, mock_resolve):
        ...
        session_id = "s1"
        mock_http.get(f"/v1/projects/p1/sessions/{session_id}/messages").respond(
            json={
                "data": [
                    {
                        "id": "e1",
        ...
        msgs = await async_client.sessions.messages(session_id=session_id)
    ...
    async def test_get_messages_with_cursor(
        ...
        mock_http.get("/v1/projects/p1/sessions/s1/messages").respond(
            json={
                "data": [],
                "next_cursor": None,
                "has_more": False,
        ...
        msgs = await async_client.sessions.messages(session_id="s1", cursor="c1")
        assert len(msgs.data) == 0

    @pytest.mark.asyncio
    async def test_get_session_facts(self, async_client, mock_http, mock_resolve):
        """GET /sessions/{id}/facts returns paginated facts."""
        route = mock_http.get("/v1/projects/p1/sessions/s1/facts").respond(
            json={
                "data": [
                    {
                        "id": "f1",
        ...
        result = await async_client.sessions.facts("s1", limit=25)

        request = route.calls.last.request
        assert request.url.params["limit"] == "25"
        assert isinstance(result.data[0], FactResponse)
        assert result.data[0].content == "Alice likes hiking"

    @pytest.mark.asyncio
    async def test_get_session_facts_with_cursor(
        self, async_client, mock_http, mock_resolve
    ):
        """GET /sessions/{id}/facts passes cursor param."""
        route = mock_http.get("/v1/projects/p1/sessions/s1/facts").respond(
            json={
                "data": [],
                "next_cursor": None,
        ...
        await async_client.sessions.facts("s1", cursor="c9")

        assert route.calls.last.request.url.params["cursor"] == "c9"
