...
class TestUsersClient:
    ...
    async def test_create_user(self, async_client, mock_http):
        ...
        mock_http.post("/v1/users").respond(
            json={
                "id": "u1",
                "external_id": "alice",
                "name": "Alice",
    ...
    @pytest.mark.asyncio
    async def test_create_user_omits_permissions_when_unset(
        self, async_client, mock_http
    ):
    ...
    async def test_get_user(self, async_client, mock_http):
        ...
        user_id = "u1"
        mock_http.get(f"/v1/users/{user_id}").respond(
            json={
                "id": user_id,
                "external_id": "alice",
                "name": "Alice",
    ...
    async def test_update_user(self, async_client, mock_http):
        ...
        user_id = "u1"
        mock_http.patch(f"/v1/users/{user_id}").respond(
            json={
                "id": user_id,
                "external_id": "alice",
                "name": "Alice Updated",
    ...
    async def test_list_users(self, async_client, mock_http):
        ...
        mock_http.get("/v1/users").respond(
            json={
                "data": [
                    {
                        "id": "u1",
