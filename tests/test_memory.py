"""Tests for the OpenZep Python SDK — memory domain."""
...
class TestMemoryClient:
    ...
    async def test_ingest_memory(self, async_client, mock_http):
        ...
        mock_http.post(f"/v1/users/{user_id}/memory").respond(
        ...
        result = await async_client.memory.ingest(user_id=user_id, messages=messages)
    ...
    async def test_get_context(self, async_client, mock_http):
        ...
        mock_http.get(f"/v1/users/{user_id}/context").respond(json=expected)

        result = await async_client.memory.get_context(
            user_id=user_id, query="hello", limit=10
        )
    ...
    async def test_delete_memory(self, async_client, mock_http):
        ...
        user_id = "user-123"
        mock_http.delete(f"/v1/users/{user_id}/memory").respond(status_code=204)

        await async_client.memory.delete(user_id=user_id)
    ...
    async def test_ingest_memory_validation_error(self, async_client, mock_http):
        ...
        with pytest.raises(Exception):
            await async_client.memory.ingest(user_id=user_id, messages=[])
